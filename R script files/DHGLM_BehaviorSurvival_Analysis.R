...
saveRDS(hardwood_brm, "Final_Scripts_data/hardwood_riiv_FINAL.rds")
...
saveRDS(shrub_brm, "Final_scripts_data/shrub_riiv_FINAL.rds")
...
sur <- read_csv("survival_meta_02_17_2026_FINAL.csv")
survival_final <- sur %>%
  filter(Status_clean %in% c("resident", "transient")) %>%
  group_by(COD) %>%
  filter(!COD %in% c("Unknown", "Vehicle", "Distemper"))
